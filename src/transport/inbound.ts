...
import { peerFingerprint } from '../security.js';
...
export async function handleInbound(
  ...
  if (!agentBody) {
    if (_config.debug) console.log('[im-qqbot] [pipeline] ignored empty inbound message');
    return;
  }

  const peer = peerFingerprint(peerId);
  if (_config.logMessageContent) {
    logger.info(`Processing: scope=${scope} peer=${peer} body="${agentBody.slice(0, 200)}"`);
  } else {
    logger.info(`Processing: scope=${scope} peer=${peer} chars=${agentBody.length}`);
  }

  // ── 获取或创建会话 ──
  let record;
  try {
    if (_config.debug) console.log('[im-qqbot] [pipeline] creating or resuming DSH session');
    record = await manager.getOrCreate(scope, peerId, msg.senderId, replyTarget);
    if (_config.debug) console.log(`[im-qqbot] [pipeline] DSH session ready id=${record.sessionId}`);
  } catch (err) {
    const detail = err instanceof Error ? err.message : String(err);
    logger.error(`ERROR creating session: ${detail}`);
    if (_config.debug) console.error(`[im-qqbot] [pipeline] DSH session failed: ${detail}`);
    return;
  }
  ...
  if (_config.debug) console.log('[im-qqbot] [pipeline] prompt submitted to DSH agent');
  logger.info(`→ followup sent: scope=${scope} peer=${peer}`);
}
